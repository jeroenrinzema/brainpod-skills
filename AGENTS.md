...
- **The tools the CLI drives are where skill content earns its place.** `image build` execs Docker Buildx and a pinned Railpack; their behaviour reaches the user through `brainpod` while `describe` says nothing about it, and no amount of reading the CLI's flags reveals it. Which stream a wrapped tool prints on, which of its own configuration files or environment variables the CLI does and does not pass through, and what its output leaves the container filesystem looking like are all fair game — and are usually the difference between a deploy that works and one that fails at runtime. Verify each against the pinned version, and name that version in the PR, not in the skill file.

- **Do not teach the agent to re-derive a decision the CLI already makes.** The CLI picks the builder, resolves the platform against live clusters, and lets Railpack detect the stack. A skill that walks the agent through reaching the same conclusion adds a second implementation that will drift from the real one. Say who decides, what the decision obliges the agent to do downstream, and where to read the result back — then stop.
...
- **Every line must earn its place.** Add only what's useful or what an agent couldn't infer on its own. Cut filler, restatements, self-explanatory steps, and anything the agent will already have from `describe` or task context. Keep new use-case references at 225 lines or fewer, including frontmatter; this is a maximum, and if you exceed it you are likely producing slop. `deploy.md` is the standing exception and already sits above the budget — it spans the whole first-deploy path. That is not a licence to keep growing it: state the line count either side of any edit to it, and be able to say why what you added outweighs the concision it costs. "It is all true" is not that argument.
...
- **Do not invent platform behaviour.** If you cannot confirm something from the CLI source, `describe`, or the CLI README, either leave it out or say plainly that it is unverified. Wrong specifics are worse than absent ones, because the agent will act on them. Confirming means reading the source: clone [brainpodnl/cli](https://github.com/brainpodnl/cli) and follow the code path you are describing. The README states intent, and intent is exactly what a skill file must not paraphrase.
...
- **Verified against the CLI.** For every command, flag, resource field, default, or error code the diff asserts, confirm it against [brainpodnl/cli](https://github.com/brainpodnl/cli) — its README and its source. Flag anything unverifiable, and flag restatements of the flag surface that `describe` already provides. Claims about a wrapped tool — Docker Buildx, Railpack — must hold against the version the CLI pins today, so check the pin too: a claim that was true of an older release is a wrong specific, not a stale one.
- **Nothing re-derives what the CLI decides.** Flag guidance that has the agent reaching a conclusion the CLI already reaches for it, such as choosing a builder or a platform.
...
- **Is the reference ruthlessly concise?** Flag filler, self-explanatory instructions, repeated guidance, and details the agent will already have from `describe` or task context. References should be at most 225 lines including frontmatter; anything longer needs specific, convincing justification. For a file already over budget, check the line count either side of the diff and require the author to have justified the growth line by line.
